...
RSpec.describe TheCoctailDbAdapter do
  describe '#random' do
    it 'returns JSON response from external API' do
      api_response = JSON.parse(
        Rails.root.join('spec/fixtures/random_coctail_response.json').read
      )
      allow(HTTParty).to receive(:get).and_return(api_response)

      expect(described_class.random).to eq(api_response['drinks'].first)
    end
  end
  ...
  describe '#meal_by_id' do
    it 'returns JSON response from external API' do
      ...
      expect(described_class.meal_by_id(id: 52_854)).to eq(api_response['drinks'].first)
    end
  end
  ...
  describe '#search' do
    it 'returns JSON response from external API' do
      ...
      expect(described_class.search(query: 'p')).to eq(api_response['drinks'])
    end
